...
void PolynomialFloat32ChebyshevMallocAllocate(PolynomialFloat *polynomial_list, size_t n) {
    if (!polynomial_list) {
        polynomial_list = (PolynomialFloat *)malloc(sizeof(PolynomialFloat) * (n + 1));
    }
    PolynomialFloat *this_polynomial = polynomial_list;
    for (size_t i = 1; i < n + 2; ++i) {
...
void PolynomialFloat32ChebyshevFree(PolynomialFloat *chebyshev_polynomials, size_t chebyshev_degree) {
    for (size_t i = 0; i < chebyshev_degree + 1; ++i) {
        free(chebyshev_polynomials[i].coefficients);
    }
    free(chebyshev_polynomials);
}
