...
void StringAppend(StringBuffer *buff, const char *new_string) {
  ...
  if (buff->capacity + new_string_size > MAX_STRING_SIZE) {
    perror("String exceeds allowable size");
    exit(EXIT_FAILURE);
  }
...
void ParseDate(const char *date, int *year, int *month, int *day) {
  ...
  if (strlen(date) != expected_len) {
    perror("Unsupported date format - wrong size");
    exit(EXIT_FAILURE);
  }
...
void CastIntTo2Chars(char *output, int input) {
  if ((input < 0) || (input > 99)) {
    perror("Invalid number passed to CastIntTo2Chars");
    exit(EXIT_FAILURE);
  } else if (input < 10) {
    output[0] = '0';
...
int ReadLine(FILE *handler, char *buffer, int max_size) {
  ...
  while ((this_char = getc(handler)) != '\n') {
    if (buff_size == max_size) {
      printf("ReadLine - buffer too small");
      exit(1);
    }
  ...
  while (char_idx <= n_chars) {
    ...
    if ((this_char == ',') && (open_quote == 0)) {
      ...
      if (buffer_size + 1 == max_buff_size) {
        ...
        printf("Buffer has reached max size");
        exit(1);
      }
  ...
  switch (attr_name) {
    ...
    case ACTION_TYPE:
      if (strcmp(attr_value, "NEW") == 0) {
        swap_p->action_type = Swap::NEW;
      } else if (strcmp(attr_value, "CORRECT") == 0) {
        swap_p->action_type = Swap::CORRECT;
      } else if (strcmp(attr_value, "CANCEL") == 0) {
        swap_p->action_type = Swap::CANCEL;
      } else {
        swap_p->action_type = Swap::ACTION_ERROR;
      }
      break;
    case TRANSACTION_TYPE:
      if (strcmp(attr_value, "Trade") == 0) {
        swap_p->transaction_type = Swap::TRADE;
      } else if (strcmp(attr_value, "Amendment") == 0) {
        swap_p->transaction_type = Swap::AMENDMENT;
      } else if (strcmp(attr_value, "Termination") == 0) {
        swap_p->transaction_type = Swap::TERMINATION;
      } else {
        swap_p->transaction_type = Swap::TRANSACTION_ERROR;
      }
      break;
    case IS_BLOCK_TRADE:
      if (strcmp(attr_value, "Y") == 0) {
        swap_p->is_block_trade = Swap::Y;
      } else if (strcmp(attr_value, "N") == 0) {
        swap_p->is_block_trade = Swap::N;
      } else {
        swap_p->is_block_trade = Swap::BLOCK_TRADE_ERROR;
      }
      break;
    case VENUE:
      if (strcmp(attr_value, "OFF") == 0) {
        swap_p->venue = Swap::OFF;
      } else if (strcmp(attr_value, "ON") == 0) {
        swap_p->venue = Swap::ON;
      } else {
        swap_p->venue = Swap::VENUE_ERROR;
      }
      break;
    case CURRENCY:
  ...
  while (current_input_idx < input_size)
  {
           ...
           (current_input_idx < input_size))
    {
      ...
      if (buff_idx > max_buff_size) {
        printf("Max buffer size reached in ListStringToSwap");
        exit(1);
      }
                                 ...
                                 int max_n_loaded_swaps)
{
  ...
  swap_list.contents = (Swap *)malloc(max_n_loaded_swaps * sizeof(Swap));
  colnames.contents = (char *)malloc(max_n_cols * max_colname_len);
